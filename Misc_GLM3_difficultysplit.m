...
%easy hard median split desnt work on GLM3 cause not enough data, so what
%if we don't median split, but only look at one difficuty and look at
%easiest/hardest 70% or so?
...
for partic=1:length(Partic)
    ...
    %% median split(HV-LV)
    onlyhard=zeros(size(hv_v));
    onlyhard(hv_v-lv_v<quantile(hv_v-lv_v, .75))=1;
    onlyeasy=zeros(size(hv_v));
    onlyeasy(hv_v-lv_v>quantile(hv_v-lv_v, .25))=1;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for run=1:length(Conds)
            ...
            if EASY
                ...
                idx=(onlyeasy==1 & session==this_session & tms==this_tms  & (position(:,pos_oi)==locations{this_loc+1}(1) | position(:,pos_oi)==locations{this_loc+1}(2) ));
            else
                idx=(onlyhard==1 & session==this_session & tms==this_tms  & (position(:,pos_oi)==locations{this_loc+1}(1) | position(:,pos_oi)==locations{this_loc+1}(2) ));
            end
            if sum(idx)==0
                ...
                if EASY
                    ...
                    [betas_easy(partic,:,run),dev,stats]=glmfit(regressors,criterion,'binomial');
                else
                    [betas_hard(partic,:,run),dev,stats]=glmfit(regressors,criterion,'binomial');
                end
...
%% TTEST
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ttest of MIP1contra and MIP0contra d_hV against 0
%only non-tms
if EASY
    betas=betas_easy;
    disp('EASY')
...
for regressor=size(betas,2)
    for cond = 5:6%{'MIP0contra'}    {'MIP1contra'}
        ...
        fprintf('\nTesting %s against 0:\n',Regs{regressor-1})
        fprintf('-%s: %s: t(%d) = %2.2f, p = %2.3f, d = %2.2f, CI = [%2.2f, %2.2f]\n',Regs{regressor-1},Conds{cond},stats.df,stats.tstat, p,D,CI)
    end
end
