%% Plot VBM Correlations
% Creates figure 4 in manuscript:
% correlations between MT/MIP grey matter and MT/MIP effect
...
clear
corr_or_partialcorr=1;

%load MIP GM
filename = 'D:\PolyU\TMS\Results\VBM\output\GM_MIP.txt';
...
%load MT GM
filename = 'D:\PolyU\TMS\Results\VBM\output\GM_MT.txt';
delimiter = ' ';
formatSpec = '%s%[^\n\r]';
fileID = fopen(filename,'r');
dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter, 'MultipleDelimsAsOne', true,  'ReturnOnError', false);
...
%load MIP design
filename = 'D:\PolyU\TMS\Results\VBM\output\MIP_Test_Design.txt';
delimiter = '\t';
formatSpec = '%*s%f%f%f%[^\n\r]';
fileID = fopen(filename,'r');
...
%load MT design
filename = 'D:\PolyU\TMS\Results\VBM\output\MT_Test_Design.txt';
delimiter = '\t';
formatSpec = '%*s%f%f%f%[^\n\r]';
fileID = fopen(filename,'r');
dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter,  'ReturnOnError', false);
...
if corr_or_partialcorr==1
    %correlations
    [r p]=corr(MIP,D_MIP);
    % [r p] = corr(MIP,D_MIP, 'type', 'Spearman')
